...
class TimerSystem
{
	...
	private:
		...
		void TariffsExpireHandler(MessageSystem& message_system) const noexcept;
		void WeaknessExpireHandler(MessageSystem& message_system) const noexcept;
		...
		void EnemyFreezeExpireHandler(MessageSystem& message_system) const noexcept;
		...
		static constexpr std::array<void(TimerSystem::*)(MessageSystem&) const noexcept, static_cast<size_t>(Timer::COUNT)> TimeoutHandlers =
		{
			...
			&TimerSystem::TariffsExpireHandler,
			&TimerSystem::WeaknessExpireHandler,
			...
			&TimerSystem::EnemyFreezeExpireHandler
		};
};
